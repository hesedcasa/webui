...
- CI: `.github/workflows/run-e2e-tests.yml` runs both legs on demand only (`workflow_dispatch`), like every other e2e workflow in these repos. Nothing here requires secrets, so a manual run needs no setup: without `.env` the plugin UI-leg skips entirely.
