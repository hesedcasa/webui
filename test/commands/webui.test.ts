...
import type WebUICommand from '../../src/commands/webui.js'
...
const fwdSlash = (p: string) => p.split(path.sep).join('/')
...
async function loadWebUI(port = 14_040) {
  const {default: WebUI} = await esmock(fwdSlash(path.join(__dirname, '../../src/commands/webui.js')), {
    [fwdSlash(path.join(__dirname, '../../src/lib/server.ts'))]: {startServer: mockStartServer(port)},
  })
  return WebUI as typeof WebUICommand
}
